...
deploy_minimal() {
    ...
    deploy_file "$BMAD_ROOT/scripts" "$TARGET/scripts" "scripts directory"
    echo ""
...
deploy_full() {
    ...
    deploy_file "$BMAD_ROOT/scripts" "$TARGET/.bmad-enhanced/scripts" "scripts"
    ...
    if [ "$DRY_RUN" != true ]; then
        ...
        echo -e "${GREEN}✅${NC} Created symlink: scripts -> .bmad-enhanced/scripts"
...
if [ "$DRY_RUN" != true ]; then
    ...
    echo "   ${YELLOW}python3 scripts/monitor-skills.py${NC}"
    echo ""
